...
```{r , include = FALSE}
# Regla 3 sigma
regla_sigma <- function(x){
  rango_1 <- mean(x,na.rm=TRUE) -3*sd(x, na.rm=TRUE)
  rango_2 <- mean(x,na.rm=TRUE) +3*sd(x, na.rm=TRUE)
  outliers <- ifelse((x < rango_1) | (x > rango_2), TRUE, FALSE)
  return(outliers)
}

# Identificador Hampel
reglahampel <- function(x){
  rango_1 <- mean(x,na.rm=TRUE) -3*MADM(x)
  rango_2 <- mean(x,na.rm=TRUE) +3*MADM(x)
  outliers <- ifelse((x < rango_1) | (x > rango_2), TRUE, FALSE)
  return(outliers)
}
...
reglapercentil <- function(x){
  ...
  outliers <- ifelse((x < q_bajo-1.5*iqr) | (x > q_alto+1.5*iqr), TRUE, FALSE)
  return(outliers)
}
```
