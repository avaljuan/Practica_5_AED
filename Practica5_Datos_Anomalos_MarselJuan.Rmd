...
Oberservamos que hay valores `Inf` y también `NA`.
...
numero_completas <- sum(complete.cases(dirty_iris))
numero_completas

porcentaje_completas <- (numero_completas/nrow(dirty_iris))*100
porcentaje_completas
...
```{r , include = TRUE}
MADM <- function (col){
 return( 1.4826*median(abs(col-median(col,na.rm=TRUE)),na.rm=TRUE))
}
...
stats_clean %>% 
  select(grep(colnames(stats_clean),pattern="Petal[.]Length.*", value=TRUE))
...
reglahampel <- function(x){
  rango <- 3*MADM(x)
  outliers <- ifelse(is.na(x),NA,ifelse((x >rango), TRUE, FALSE))
  return(outliers)
}
...
reglaboxplot <- function(x){
  ...
  outliers <- ifelse(is.na(x),NA,ifelse((x < q1-1.5*iqr) | (x > q3+1.5*iqr), TRUE, FALSE))
  return(outliers)
}
# Percentiles
reglapercentil <- function(x){
    iqr <- IQR(x, na.rm=TRUE)
  q_bajo <- quantile(x,probs=0.05, na.rm=TRUE)
  q_alto <- quantile(x, probs=0.95, na.rm=TRUE)
  outliers <- ifelse(is.na(x),NA,ifelse((x < q_bajo-1.5*iqr) | (x > q_alto+1.5*iqr), TRUE, FALSE))
  return(outliers)
}
```
...
dirty_deteccion_outliers <- dirty_iris %>% 
  mutate(
    across(
      where(is.numeric),
      list(
        outliers_sigma       = ~ reglasigma(.),
        outliers_hampel      = ~ reglahampel(.),
        outliers_boxplot     = ~ reglaboxplot(.),
        outliers_percentiles = ~ reglapercentil(.)
      )
    )
  )


dirty_outliers_petlen<- dirty_deteccion_outliers %>% 
  select(grep(pattern = "Petal[.]Length.*", colnames(dirty_deteccion_outliers), value = TRUE))
```
...
dirty_solo_outliers <- dirty_outliers_petlen %>% 
  filter(if_any(-Petal.Length,~.==TRUE)) %>% 
  arrange(desc(Petal.Length))
```
